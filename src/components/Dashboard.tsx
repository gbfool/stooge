...
const Dashboard = () => {
  ...
  const [currDeletingFileId, setCurrDeletingFileId] = useState<string | null>(
    null
  );
  ...
  const { mutate: deleteFileMutation } = trpc.deleteFile.useMutation({
    ...
    onMutate: ({ id }) => {
      setCurrDeletingFileId(id);
    },
    onSettled: () => {
      setCurrDeletingFileId(null);
    },
  });
  return (
    <main className="m-auto max-w-7xl md:p-10">
